...
const Upload = () => {
  ...
  const mindDropzone = useDropzone({ onDrop: onDropMind, accept: { "application/octet-stream": [".mind"] }, maxFiles: 1 });
  ...
  const handleSubmit = async () => {
    ...
    // Simula salvamento (sem backend por enquanto)
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div className="bg-card border border-primary/20 rounded-xl p-6 border-glow">
              ...
              <ol className="text-sm text-muted-foreground space-y-2 mb-4">
                <li className="flex gap-2">
                  ...
                  Clique no botão abaixo para abrir o compilador
                </li>
                <li className="flex gap-2">
                  <span className="text-primary font-bold">2.</span>
                  Faça upload da mesma imagem target
                </li>
                <li className="flex gap-2">
                  <span className="text-primary font-bold">3.</span>
                  Baixe o arquivo <code className="text-primary">.mind</code> gerado
                </li>
                <li className="flex gap-2">
                  <span className="text-primary font-bold">4.</span>
                  Volte aqui e faça upload do <code className="text-primary">.mind</code>
                </li>
              </ol>
              <a
            ...
            <DropZone
              ...
              file={mindFile ? { file: mindFile, preview: "" } : null}
              type="file"
            />
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setStep(1)}
                className="flex-1 border border-border text-muted-foreground font-display font-bold text-sm tracking-wider py-4 rounded-lg hover:border-primary/30 hover:text-foreground transition-all"
              >
                VOLTAR
              </button>
              <button
                ...
                className="flex-1 bg-primary text-primary-foreground font-display font-bold text-sm tracking-wider py-4 rounded-lg glow hover:glow-strong transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:shadow-none"
              >
                PRÓXIMO
              </button>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Imagem Target</span>
                  <span className="text-primary">{targetImage?.file.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Vídeo</span>
                  <span className="text-primary">{video?.file.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Target .mind</span>
                  <span className="text-primary">{mindFile?.name}</span>
                </div>
              </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-primary text-primary-foreground font-display font-bold text-sm tracking-wider py-4 rounded-lg glow hover:glow-strong transition-all disabled:opacity-50"
              >
                {isSubmitting ? (
    ...
    >
      ...
      {file ? (
        <div className="flex items-center gap-4">
          ...
          {type === "video" && preview && (
            <video src={preview} className="w-16 h-16 rounded-lg object-cover border border-border" muted />
          )}
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-muted-foreground">
              {(file.file.size / 1024 / 1024).toFixed(1)} MB — Toque para trocar
            </p>
          </div>
